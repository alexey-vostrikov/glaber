...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		echo "Total init files processed: $processed_files"
		echo 
		if [ "$processed_files" -ne 0 ]; then
		  echo "Restarting mysql to apply init files changes"
			echo
		  ...
		  echo "All init files processed, no mysql restart needed"
			echo 
		fi
