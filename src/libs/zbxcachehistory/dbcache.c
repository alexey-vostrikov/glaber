...
//				*more = ZBX_SYNC_MORE;
	...
	} while ( (history_num + triggers_num + timers_num) > 0  && ZBX_HC_SYNC_TIME_MAX >= time(NULL) - sync_start);
...
int init_database_cache(char **error)
{
	...
	//if (SUCCEED != (ret = zbx_shmem_create(&hc_mem, 16 * ZBX_MEBIBYTE, "history cache",
