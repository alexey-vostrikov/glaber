...
static int convert_metric_to_value(glb_state_item_value_t *cache_val, metric_t *metric, unsigned char value_type)
{
    ...
    {
    case ITEM_VALUE_TYPE_FLOAT:
        zbx_variant_set_dbl(&cache_val->value, metric->value.data.dbl);
        break;

    case ITEM_VALUE_TYPE_UINT64:
        zbx_variant_set_ui64(&cache_val->value, metric->value.data.ui64);
        break;

    case ITEM_VALUE_TYPE_STR:
    case ITEM_VALUE_TYPE_TEXT:
    {
        char *tmp_str = metric->value.data.str;
        if (NULL == tmp_str)
            tmp_str = "";

        int len = strlen( metric->value.data.str) + 1;
        char *str = state->memf.malloc_func(NULL, len);
    ...
    case ITEM_VALUE_TYPE_LOG:
    {
        if (NULL != metric->value.data.str)
        {
            ...
            memcpy(str, metric->value.data.str, len);
            zbx_variant_set_str(&cache_val->value, str);
        }
        else
        {
    ...
    case ITEM_VALUE_TYPE_NONE:
        ...
        {
            int len = strlen(metric->value.data.err) + 1;
            char *str = state->memf.malloc_func(NULL, len);
            memcpy(str, metric->value.data.err, len);
            zbx_variant_set_error(&cache_val->value, str);
        }
        break;

    default:
        LOG_WRN("Wrong data type %d", value_type);
        THIS_SHOULD_NEVER_HAPPEN;
        exit(-1);
    }
    cache_val->time_sec = metric->sec;
}
...
static int fetch_from_db_by_count(u_int64_t itemid, item_elem_t *elm, int count, int head_time, int now )
{
    ...
    if (SUCCEED == (ret = glb_history_get_history(itemid, elm->value_type, 0, count, head_time, HISTORY_GET_NON_INTERACTIVE, &values)))
    {
...
static int glb_state_fetch_from_db_by_time(u_int64_t itemid, item_elem_t *elm, int seconds, int head_time, int now)
{
    ...
    if (SUCCEED == (ret = glb_history_get_history(itemid, elm->value_type, head_time - seconds, 0,
                                          elm->db_fetched_time, HISTORY_GET_NON_INTERACTIVE, &values)))
    {
        ...
        if (values.values_num > 0)
        {
            ...
            for (i = 0; i < values.values_num; i++)
            {
                ...
                {
                    DEBUG_ITEM(itemid, "WARN: COULDN'T add item with timestamp %d, most recenet time in the cache is %d",
                               values.values[i].timestamp.sec, glb_tsbuff_get_time_head(&elm->tsbuff));
                }
            }
...
int add_value_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *data)
{
    ...
    elm = (item_elem_t *)elem->data;
    add_val_req_t *req= data;
    metric_t *metric = req->metric;
    
    unsigned value_type = req->value_type;
    ...
    if (elm->value_type == ITEM_VALUE_TYPE_NONE) {
        elm->value_type = value_type;
    }
    else if (elm->value_type != value_type)
    {
        DEBUG_ITEM(elem->id, "Resetting cache value type to %d", value_type);
        LOG_DBG("Resetting value type %d -> %d ", elm->value_type, value_type);
        free_item(elm);
        item_create_cb(elem, memf, NULL);
        elm = (item_elem_t *)elem->data;
        elm->value_type = value_type;
    ...
    if (metric->sec > now + 300)
    {
        LOG_INF("Warn: item's %ld timestamp %ld is too much ahead of the real time %ld, not adding to the cache", elem->id, metric->sec, now);
        THIS_SHOULD_NEVER_HAPPEN;
        //  exit(-1);
        return FAIL;
    }

    if (NULL != (c_val = (glb_state_item_value_t *)glb_tsbuff_add_to_head(&elm->tsbuff, metric->sec)))
    {
        convert_metric_to_value(c_val, metric, value_type);

        DEBUG_ITEM(elem->id, "Item added, most recent timestamp is %ld", glb_tsbuff_get_time_head(&elm->tsbuff));
    }
    else
    {
        LOG_WRN("Cannot add value for item %ld with timestamp %d to the cache", metric->itemid, metric->sec);
        return FAIL;
    }
...
static int item_update_meta_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *cb_data)
{
    ...
    //    LOG_INF("Updating metadata: errmsg new %s", req->meta->error);
     //   LOG_INF("Updating metadata: errmsg old %s", elm->meta.error);
        strpool_replace(&state->strpool, &elm->meta.error, req->meta->error);
    }
...
static int item_get_meta_cb(elems_hash_elem_t *elem, mem_funcs_t *memf,  void *cb_data)
{
    ...
    strpool_replace(&state->strpool, &meta.error, elm->meta.error);
...
static int json_to_metric(struct zbx_json_parse *jp, unsigned char value_type, metric_t *metric)
{
    int errflag = 0;
    size_t alloc = 0;
    ...
    if (FAIL == (metric->sec = glb_json_get_int_value_by_name(jp, "clock", &errflag)))
        return FAIL;

    switch (value_type)
    {
    case ITEM_VALUE_TYPE_UINT64:
        metric->value.data.ui64 = glb_json_get_int_value_by_name(jp, "value", &errflag);
        metric->value.type = VARIANT_UI64;
        break;
    case ITEM_VALUE_TYPE_FLOAT:
        metric->value.type = VARIANT_DBL;
        metric->value.data.dbl = glb_json_get_dbl_value_by_name(jp, "value", &errflag);
        break;
    case ITEM_VALUE_TYPE_STR:
    case ITEM_VALUE_TYPE_TEXT:
    case ITEM_VALUE_TYPE_LOG:
        ...
        {
            metric->value.data.str = str_value;
        }
        else
        {
    ...
    while (NULL != (value_ptr = zbx_json_next(jp, value_ptr)))
    {
        ...
        if (SUCCEED == zbx_json_brackets_open(value_ptr, &jp_value) &&
         //   SUCCEED == json_to_hist_record(&jp_value, elm->value_type, &h))
            SUCCEED == json_to_metric(&jp_value, elm->value_type, &metric))
        {
            add_val_req_t req = {.value_type=elm->value_type, .metric = &metric};
        
            add_value_cb(elem, memf,  &req);
            i++;
        }
...
int  glb_state_item_add_value( metric_t *metric, unsigned char value_type) {
    int i, ret = SUCCEED;

    LOG_DBG("In %s: starting adding new history value", __func__);

    DEBUG_ITEM(metric->itemid, "Adding to value cache, flags is %d state is %d, timestamp is %d", metric->flags, metric->state, metric->sec);
    
    if (0 != ((ZBX_DC_FLAG_NOVALUE | ZBX_DC_FLAG_UNDEF) & metric->flags) || ITEM_STATE_NOTSUPPORTED == metric->state)
    {
        DEBUG_ITEM(metric->itemid, "Not adding to value cache, no_hist flag is set");
        return FAIL;
    }
    add_val_req_t req = {.value_type = value_type, .metric = metric};
    if (FAIL == elems_hash_process(state->items, metric->itemid, add_value_cb, &req, 0))
    {
        ret = FAIL;
    };
    
    DEBUG_ITEM(metric->itemid, "Added value with timestamp %d to the cache", metric->sec);
};
...
ELEMS_CALLBACK(set_error_cb) {
    item_elem_t* elm  = elem->data;    
    char *errorstr = data;
    int new_state = ITEM_STATE_NOTSUPPORTED;

    if (strlen(errorstr) < 1) 
        new_state = ITEM_STATE_NORMAL;

    elm->meta.state = new_state;
    strpool_replace(&state->strpool, &elm->meta.error,  errorstr);
    
    return SUCCEED;
}

int  glb_state_item_set_error(u_int64_t itemid, char *errorstr) {
    if (NULL == errorstr)
        errorstr = "";
    return elems_hash_process(state->items, itemid, set_error_cb, errorstr, 0);
}
