...
#include "metric.h"
#include "pp_execute_throttle.h"
#include "pp_execute_dispatch.h"
 ...
 *                                                                            *
 ******************************************************************************/
int	pp_execute_step(zbx_pp_context_t *ctx, const metric_t *metric_in, zbx_pp_cache_t *cache, unsigned char value_type,
		zbx_variant_t *value, zbx_pp_step_t *step, zbx_variant_t *history_value,
		zbx_timespec_t *history_ts)
{
	int	ret;

	DEBUG_ITEM( metric_in->itemid, "Processing step:%d params:'%s' value:'%s' cache:%p", 
	...
	switch (step->type)
	{
		...
		case ZBX_PREPROC_DELTA_VALUE:
		case ZBX_PREPROC_DELTA_SPEED:
			ret = pp_execute_delta(step->type, value_type, value, metric_in->ts, history_value, history_ts);
			goto out;
		...
		case ZBX_PREPROC_THROTTLE_VALUE:
			ret = item_preproc_throttle_value(value, &metric_in->ts, history_value, history_ts);
			goto out;
		case ZBX_PREPROC_THROTTLE_TIMED_VALUE:
			ret = pp_throttle_timed_value(value, metric_in->ts, step->params, history_value, history_ts);
			goto out;
		...
		case GLB_PREPROC_THROTTLE_TIMED_VALUE_AGG:
			ret = pp_execute_throttle_value_agg(value, &metric_in->ts, step->params, history_value, history_ts, value_type);
			goto out;
		case GLB_PREPROC_DISPATCH_ITEM:
			ret = pp_execute_dispatch(metric_in, value, step->params);
			goto out;
		case GLB_PREPROC_DISPATCH_ITEM_BY_IP:
			ret = pp_execute_dispatch_by_ip(metric_in, value, step->params);
			goto out;	
		case GLB_PREPROC_JSON_FILTER:
			ret = pp_execute_json_filter(metric_in->itemid, value, &metric_in->ts, step->params, history_value, history_ts, value_type);
			goto out;
		case GLB_PREPROC_DISCOVERY_PREPARE:
			ret = pp_execute_json_discovery_prepare(metric_in->itemid, value, &metric_in->ts, step->params, history_value, history_ts, value_type);
			goto out;
		default:
			zbx_variant_clear(value);
			zbx_variant_set_error(value, zbx_dsprintf(NULL, "unknown preprocessing step %d", step->type));
			ret = FAIL;
		}
out:
...
void	pp_execute(zbx_pp_context_t *ctx, zbx_pp_item_preproc_t *preproc, zbx_pp_cache_t *cache,
		const metric_t *metric_in, zbx_variant_t *value_out, zbx_pp_result_t **results_out,
		int *results_num_out)
{
	...
	DEBUG_ITEM(metric_in->itemid, "Preprocessing metric, in value %s, type %s",
			zbx_variant_value_desc(NULL == cache ? &metric_in->value : &cache->value),
			zbx_variant_type_desc(NULL == cache ? &metric_in->value : &cache->value));

	if (NULL == preproc || (0 == preproc->dep_itemids_num && 0 == preproc->steps_num))
	{
		zbx_variant_copy(value_out, NULL != cache ? &cache->value : &metric_in->value);
		
		DEBUG_ITEM(metric_in->itemid, "Finished preprocessing metric, preprocessing isn't needed, returning initial value %s, type %s",
	...
	if (NULL == cache)
	{
		zbx_variant_copy(value_out, &metric_in->value);
	}
	else
	{
	...
	for (int i = 0; i < preproc->steps_num; i++)
	{
		...
		DEBUG_ITEM(metric_in->itemid, "Preprocessing step %d", i);
		...
		if (SUCCEED != pp_execute_step(ctx, metric_in, cache, preproc->value_type, value_out, preproc->steps + i,
	...
	if (ZBX_VARIANT_ERR == value_out->type)
	{
		...
		if (0 != results_num && ZBX_PREPROC_FAIL_SET_ERROR != action && 0 == quote_error)
		{
			...
			pp_format_error(&metric_in->value, results, results_num, &error);
...
out:
	DEBUG_ITEM(metric_in->itemid, "Finished preprocessing metric, out value %p %s, type %s",value_out,
				zbx_variant_value_desc(value_out), zbx_variant_type_desc(value_out));
}
