						 ...
						 zbx_hashset_t *activated_hosts)
{
	...
	while (SUCCEED == (ret = zbx_dbsync_next(sync, &rowid, &row, &tag)))
	{
		...
		else if ((HOST_STATUS_PROXY_ACTIVE == status || HOST_STATUS_PROXY_PASSIVE == status || HOST_STATUS_SERVER == status) &&
				 (0 == found || 0 != strcmp(host->host, row[2])))
		{
			if (1 == found)
			{
		...
		if (HOST_STATUS_PROXY_ACTIVE == status || HOST_STATUS_PROXY_PASSIVE == status || HOST_STATUS_SERVER == status || HOST_STATUS_DOMAIN == status)
		{
			...
			if ((HOST_STATUS_SERVER == status || HOST_STATUS_PROXY_PASSIVE == status) && 
			...
			else if ((HOST_STATUS_PROXY_ACTIVE == status || HOST_STATUS_DOMAIN == status) && ZBX_LOC_QUEUE == proxy->location)
			{
				zbx_binary_heap_remove_direct(&config->pqueue, proxy->hostid);
				proxy->location = ZBX_LOC_NOWHERE;
			}
