...
//	dsec2 = zbx_time() - sec;
	...
	if (SUCCEED == ZBX_CHECK_LOG_LEVEL(LOG_LEVEL_INFORMATION))
	{
		...
		zabbix_log(LOG_LEVEL_INFORMATION, "%s() htmpls     : %d ", __func__,
				obj_index_get_numdata(glb_config->host_to_template_idx));
		...
		zabbix_log(LOG_LEVEL_INFORMATION, "%s() trigdeps   : %d", __func__,
				obj_index_get_numdata(glb_config->deptrigger_to_trigger_idx));
