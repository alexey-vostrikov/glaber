...
//	zabbix_log(LOG_LEVEL_INFORMATION,"Trends update1");
	if (cache->trends_last_cleanup_hour < hour && ZBX_TRENDS_CLEANUP_TIME < seconds)
//	if (1)
	{
		zbx_hashset_iter_t	iter;
		
		//zabbix_log(LOG_LEVEL_INFORMATION,"Trends update2");
		...
		while (NULL != (trend = (ZBX_DC_TREND *)zbx_hashset_iter_next(&iter)))
		{
			if (trend->clock == hour)
				continue;
		//	zabbix_log(LOG_LEVEL_INFORMATION,"Trends update3");
			if (SUCCEED == zbx_history_requires_trends(trend->value_type)) {
			//	zabbix_log(LOG_LEVEL_INFORMATION, "HIST UPD: %d",cache->trends_last_cleanup_hour);
...
static void	sync_server_history(int *values_num, int *triggers_num, int *more)
{
	...
	{
		DC_ITEM			*items;
		int			*errcodes, trends_num = 0, export_trends_num=0, timers_num = 0, ret = SUCCEED;
		zbx_vector_uint64_t	itemids;
	...
	//	zabbix_log(LOG_LEVEL_INFORMATION, "Fetched %d items from the history",history_items.values_num);		
		...
		if (0 != history_num)
		{
			...
			for (i = 0; i < history_num; i++) {
				...
				if (CONFIG_DEBUG_HOST == history[i].itemid) {
			...
			if (FAIL != (ret = DBmass_add_history(history, history_num)))
			{
				...
				//hourly this will give us list of trends to export via the history interface
				...
				//we only need host and trend data during the history export
				for ( i=0; i<trends_num; i++) {
				...
				//	DBmass_update_items(&item_diff, &inventory_values);
					...
					//not doing classic db trends upload, this is glaber, not zabbix
		...
		zbx_uint64_t value_orig, zbx_uint64_t lastlogsize, int mtime, unsigned char flags)
{
	...
	if (CONFIG_DEBUG_ITEM == itemid) 
		zabbix_log(LOG_LEVEL_INFORMATION,"Debug item: %ld added to local cache as uint" );
		...
		AGENT_RESULT *result, const zbx_timespec_t *ts, unsigned char state, const char *error)
{
	...
	if (ITEM_STATE_NOTSUPPORTED == state)
	{
		...
		if (CONFIG_DEBUG_ITEM == itemid) 
				zabbix_log(LOG_LEVEL_INFORMATION,"Debug item: %ld adding to local hist cache is state %d step 11 ",itemid,state);
		...
		if (CONFIG_DEBUG_ITEM == itemid) 
				zabbix_log(LOG_LEVEL_INFORMATION,"Debug item: %ld adding to local hist cache is state %d step 12 ",itemid,state);
...
static void	hc_add_item_values(dc_item_value_t *values, int values_num)
{
	...
	for (i = 0; i < values_num; i++)
	{
		...
		if (CONFIG_DEBUG_ITEM == item_value->itemid) {
				zabbix_log(LOG_LEVEL_INFORMATION,"Debug item: %ld added to the history cache",item_value->itemid);
		}
	}
}
