...
static int get_worker_mode(char *mode) {
    ...
    if (!strcmp(mode,"double_new_line"))  return GLB_WORKER_MODE_EMPTYLINE;
    if (!strcmp(mode,"silent"))     return GLB_WORKER_MODE_SILENT;
    return FAIL;
}
...
//command line
//extra fields. 
//params, timeout, max_calls
//todo:  move string's defs to the header file

DC_EXT_WORKER *glb_init_worker(char *config_line)
{
    ...
    if (NULL == ( worker = zbx_malloc(NULL, sizeof(DC_EXT_WORKER)))) {
        return NULL;
    }
...
static int restart_worker(DC_EXT_WORKER *worker) {
    ...
    if ( worker->pid && !kill(worker->pid,0)) {
...
static int worker_is_alive(DC_EXT_WORKER *worker) {
...
//kills on timeout
static int worker_cleanup (DC_EXT_WORKER *worker) {
    ...
    if (bytes > 0) zabbix_log(LOG_LEVEL_INFORMATION,"GARBAGE DETECTED  %d bytes of garbage from %s: %s",bytes, worker->path,buffer);
...
int glb_process_worker_request(DC_EXT_WORKER *worker, const char * request, char **responce) {
   ...
   int worker_fail=0;
    ...
    if (worker->calls++ >= worker->max_calls) {
        zabbix_log(LOG_LEVEL_INFORMATION, "%s worker %s pid %d exceeded number of calls (%d), restarting it",
    ...
    if (request) request_len=strlen(request); else {
   ...
   // zabbix_log(LOG_LEVEL_INFORMATION,"Communicating to script, to_script:%d, from script:%d",worker->mode_to_worker,worker->mode_from_worker);
    ...
    //now, let's see if we completed write in time
   ...
   // zabbix_log(LOG_LEVEL_INFORMATION, "Completed write to script, will read now, timeout is %d seconds", worker->timeout);
    ...
    while ( FAIL == zbx_alarm_timed_out() && continue_read )  {
        ...
        int read_len = read(worker->pipe_from_worker,buffer,MAX_STRING_LEN);
        //zabbix_log(LOG_LEVEL_INFORMATION,"EXEC: read %d bytes from the worker", read_len);   
        //setting trailing zero
        buffer[read_len]=0;
        ...
        //todo: either remove or switch scripting to the sync mode  
             if (EAGAIN ==errno || EWOULDBLOCK ==errno || ENODATA== errno) {
                ...
                if (wait_count++ > 1){
                     usleep(1000);
                     zabbix_log(LOG_LEVEL_INFORMATION,"Waiting for new data frome the worker");
                }
            } else {
            ...
            zabbix_log(LOG_LEVEL_DEBUG,"Read %d bytes from the worker",read_len);
        }
        if ( 0 == read_len ) {
        ...
        //zabbix_log(LOG_LEVEL_INFORMATION, "Read %d from the script",read_len);

        switch (worker->mode_from_worker) {
            case GLB_WORKER_MODE_NEWLINE:
                    if  (rbuflen > 1 &&  NULL != strstr(buffer,"\n")) {
                        continue_read = 0;
                        zabbix_log(LOG_LEVEL_INFORMATION, "Found newline char in SINLE mode, finishing reading");
                    }
                break;    
            case GLB_WORKER_MODE_EMPTYLINE:
                      if ((!strcmp(buffer,"\n")) ||  (NULL != strstr(buffer,"\n\n"))) {
...
void glb_destroy_worker(DC_EXT_WORKER *worker) {
