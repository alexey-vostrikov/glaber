...
func main() {
	...
	stdout_chan := make (chan string, 65536)
	...
	go process_messages(channel,stdout_chan)
	go stdout_writer(stdout_chan)

	server.Wait()
}

func stdout_writer(stdout_chan chan string) {
	for  message := range stdout_chan {
		fmt.Println((message))
	}
}

func process_messages(channel syslog.LogPartsChannel, stdout_chan chan string) {
	for logParts := range channel {

	//	if (nil != logParts["proc_id"]) {
	//		logParts["proc_id"] = string(string(logParts["proc_id"])).ToUpper();
	//	}
...
// 	//parms, _ := json.Marshal(os.Args);
// 	//fmt.Fprintf(os.Stderr, "Got start arguments: '%s'", parms);
