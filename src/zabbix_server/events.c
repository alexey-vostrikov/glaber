...
int	save_event_to_history(DB_EVENT *event)
{
	...
	if ( 0 < event->problem_eventid )
		zbx_json_addint64(&json, "problem_eventid", event->problem_eventid);
	...
	metric_t metric = {.flags = PROCESS_METRIC_FLAG_EVENTLOG, .hostid = 0 , .itemid = event->objectid, .value.data.str = json.buffer, 
		.value.type = VARIANT_STR, .sec = time(NULL) };
	metric_processing_data_t proc_data = {.hostid = 0, .itmeid = event->objectid, .key = "", .hostname = "", .value_type = ITEM_VALUE_TYPE_LOG};
