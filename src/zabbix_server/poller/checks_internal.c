...
int	get_value_internal(const DC_ITEM *item, AGENT_RESULT *result)
{
	...
	else if (0 == strcmp(tmp, "wcache"))			/* zabbix[wcache,<cache>,<mode>] */
	{
		...
		tmp = get_rparam(&request, 1);
		tmp1 = get_rparam(&request, 2);
		//LOG_INF("Wcache reques tmp %s tmp1 %s", tmp, tmp1);
		...
		if (0 == strcmp(tmp, "values"))
		{
			...
			// else
			// {
				SET_MSG_RESULT(result, zbx_strdup(NULL, "Not supported in Glaber"));
				goto out;
			//}
		}
		else if (0 == strcmp(tmp, "history"))
		{
			...
			// else
			// {
				SET_MSG_RESULT(result, zbx_strdup(NULL, "There is no history cache in Glaber, not supported"));
				goto out;
			//}
		}
