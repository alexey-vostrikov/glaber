	...
	for (i = 0; i < num; i++)
	{
		...
		if (SUCCEED == errcodes[i])
		{
			if (0 == add_results.values_num)
			{
				...
				zbx_preprocess_item_value(items[i].host.hostid, items[i].itemid, items[i].host.hostid, items[i].value_type,
			...
			else
			{
				...
				for (j = 0; j < add_results.values_num; j++)
				{
					...
					if (ISSET_MSG(add_result))
					{
						...
						zbx_preprocess_item_value(items[i].host.hostid, items[i].itemid, items[i].host.hostid,
