	...
	switch (sess->state) {
		...
		case POLL_CONNECT_SENT:
			...
			{	//we are using async, so it might be that the socket still not ready, if so, we just have to wait  a bit 
				...
				//zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld data send failed, aborted ",conf.items[item_idx].itemid);
			} else {
				 sess->state=POLL_REQ_SENT;
				 //zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld sending request completed ",conf.items[item_idx].itemid);
			}
			break;
		...
		case POLL_REQ_SENT:
			...
			//	zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld got response data",conf.items[item_idx].itemid);
				...
				else if (0 == received_len)
				{
					SET_MSG_RESULT(&conf.results[item_idx], zbx_dsprintf(NULL, "Received empty response from Zabbix Agent at [%s]."
						" Assuming that agent dropped connection because of access permissions.", conf.items[item_idx].interface.addr));
					conf.errcodes[item_idx] = NETWORK_ERROR;
				}
				else {
			...
			//zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld closed connection ",conf.items[item_idx].itemid);
...
int init_async_agent(const DC_ITEM *items, AGENT_RESULT *results, int *errcodes, int max_items, int max_connections) {
	...
	if (NULL == (hs = zbx_malloc(NULL, sizeof(struct async_agent_session *) * max_connections))) 	return FAIL;
...
int start_agent_connection( struct async_agent_session *sess) {
	...
	zabbix_log(LOG_LEVEL_DEBUG,"Agent starting connection ");
	...
	switch (conf.items[item_idx].host.tls_connect) {
		...
		case ZBX_TCP_SEC_TLS_PSK:
			SET_MSG_RESULT(&conf.results[item_idx], zbx_dsprintf(NULL, "A TLS connection is configured to be used with agent"
					" but support for TLS was not compiled into %s.", get_program_type_string(program_type)));
	...
	//		zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld connect FAIL ",conf.items[item_idx].itemid);
			return SUCCEED;
	} 

	//marking item and connection as in polling state now
	//zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld connect started ",conf.items[item_idx].itemid);
...
int get_values_agent_async()
{
	...
	//zabbix_log(LOG_LEVEL_INFORMATION,"awfwergwergwer");
	...
	for (i = 0; i < conf.max_items; i++) {
			...
			//	zabbix_log(LOG_LEVEL_INFORMATION,"#42 hs:%ld; 	hs_idx: %d;	snmp_sess:	%ld;	item_idx:	%d;	itemid:	%ld; Item add to list, state %d ",
			//			hs[conf.items[i].host.hostid % conf.max_connections],
			//			conf.items[i].host.hostid % conf.max_connections,0,i,
			//			conf.items[i].itemid, errcodes[i]);
			//	zabbix_log(LOG_LEVEL_INFORMATION,"Agent item %ld added to a list",conf.items[i].itemid);
	...
	for ( i=0; i<conf.max_connections; i++)  {
		...
		switch (hs[i]->state) {
			case  POLL_CONNECT_SENT:
				...
				if ( 0 != result ) {
				//	zabbix_log(LOG_LEVEL_DEBUG,"Agent item %ld connection failed (reset) result is %d",conf.items[item_idx].itemid,result);
	...
	for (i=0; i<conf.max_connections; i++) {
		...
		if ( ( hs[i]->stop_time < time(NULL) ) && 
				( (POLL_FREE != hs[i]->state ) && POLL_FINISHED != hs[i]->state)) {
			...
			//zabbix_log(LOG_LEVEL_DEBUG,"item timed out");
