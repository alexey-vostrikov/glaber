...
int	snmp_set_result(poller_item_t *poller_item, csnmp_var_t *var, AGENT_RESULT *result)
{
	...
	switch (var->type) {
    	...
    	case SNMP_TP_TIMETICKS: {
			DEBUG_ITEM(poller_get_item_id(poller_item),"Arrived 64bit unsigned value on smnp: %ld",*(long long*)var->value);
    	...
    	case SNMP_TP_GAUGE:
			...
			DEBUG_ITEM(poller_get_item_id(poller_item), "Converted item of unsigned int type value %ld", result->ui64);
			return SUCCEED;
	       	break;
