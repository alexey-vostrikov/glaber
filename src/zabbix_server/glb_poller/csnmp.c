...
static int _append_var(csnmp_pdu_t *p, csnmp_var_t v) {
    if (p->vars_len == p->vars_cap) {
        ...
        p->vars = realloc(p->vars, p->vars_cap * sizeof(csnmp_var_t));
        if (p->vars == NULL) {
            return -1;
        }
    }
...
int snmp_dec_pdu(const char *buf, int buf_len, csnmp_pdu_t *p) {
    // fprintf(stderr, "pdu:\n");
...
int snmp_enc_pdu(char **buf, int *i, int *buf_len, csnmp_pdu_t *p) {
...
int csnmp_recv_pdu(int fd, csnmp_pdu_t *p) {
    int ret = -1;
...
int csnmp_send_pdu(int fd, csnmp_pdu_t *p) {
    int ret = -1;
...
void snmp_dump_var(csnmp_var_t *v) {
    asn1_dump_oid(v->oid);
...
void csnmp_dump_pdu(const char *msg, csnmp_pdu_t *p) {
    fprintf(stderr, "%s: ver %c community %s command %-9s (%x) (%d vars) reqid %x %s %d,%d\n",  //
            ...
            p->community.b, csnmp_command_str(p->command), p->command, p->vars_len, p->req_id,   //
