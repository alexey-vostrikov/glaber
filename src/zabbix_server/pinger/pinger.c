		...
		int ping_result, char *error)
{
	...
	if (NOTSUPPORTED == ping_result)
	{
		...
		zbx_preprocess_item_value(item.host.hostid, item.itemid, item.host.hostid, item.value_type, item.flags, NULL, ts,
				item.state, error);
	}
	else
	{
		...
		zbx_preprocess_item_value(item.host.hostid, item.itemid, item.host.hostid, item.value_type, item.flags, &value, ts,
				item.state, NULL);
...
static void	get_pinger_hosts(icmpitem_t **icmp_items, int *icmp_items_alloc, int *icmp_items_count)
{
	...
	for (i = 0; i < num; i++)
	{
		...
		else
		{
			...
			zbx_preprocess_item_value(item.host.hostid, items[i].itemid, items[i].host.hostid, items[i].value_type,
