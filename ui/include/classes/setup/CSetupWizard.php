...
class CSetupWizard extends CForm {
	...
	private function stageSummary(): array {
		...
		if ($server_name !== '') {
			$table
				->addRow(null)
				->addRow(
					(new CSpan(_('Glaber server name')))->addClass(ZBX_STYLE_GREY),
	...
	private function stageInstalled() {
		...
		$message_box = null;
		$message = [
			(new CTag('h1', true, _('Congratulations! You have successfully installed Glaber frontend.')))
